...
const fs = require("fs")
const trex = fs.readFileSync('./Voice/gm.mp3')  
...
Trex.addrex({on: 'text', fromMe: false,  deleteCommand: false}, (async (message, match) => {  
    ...
    if (Config.VTYPE == 'girl') {
        ...
        if (get1.test(message.message)) {
            await message.client.sendMessage(message.jid, trex, MessageType.audio, {mimetype: 'audio/mp4', ptt:true}, {quoted: message.data})
        }
