...
const UNQ = " dont type words after command 🖲️  "
const DDO = " turn on disappering mode 🖲️  "
const ONO = " Sucsessfuly Turned on 🖲️  "
...
Trex.addrex({pattern: 'dis off ?(.*)', fromMe: true, desc: DDO,dontAddCMDList: true }, (async (message, match) => {   
        ...
        if (match[1] == '') {
            ...
            await message.client.sendMessage(message.jid,' Disapearing off 🖲️  ',MessageType.text);
        }
        else {
            return await message.client.sendMessage(message.jid, UNQ, MessageType.text);
        }
